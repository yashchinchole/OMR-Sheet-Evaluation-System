...
cv2.imshow('ekdum Final',final_image)




# cv2.drawContours(bottom, contours, -10,(255,165,0),2)
# utlis.rectContours(contours,15000)
###################################################################
...
# # print(biggestContour)
# if biggestContour1.size != 0 and biggestContour2.size != 0 and  MarksPoints.size != 0 and Percentage.size != 0 and RollNumber.size != 0 and SubjectCode.size != 0 and GradeContour.size != 0:
#     cv2.drawContours(imgBiggestContours, biggestContour1, -1, (0, 255, 0), 20)
#     cv2.drawContours(imgBiggestContours, biggestContour2, -1, (0, 255, 0), 20)

#     cv2.drawContours(imgBiggestContours, RollNumber, -1, (0, 255, 255), 10)
#     cv2.drawContours(imgBiggestContours, SubjectCode, -1, (0, 255, 255), 10)

#     cv2.drawContours(imgBiggestContours, MarksPoints, -1, (255, 0, 255), 20)
#     cv2.drawContours(imgBiggestContours, Percentage, -1, (255, 0, 255), 20)
#     cv2.drawContours(imgBiggestContours, GradeContour, -1, (255, 0, 255), 20)
...
#     pt3 = np.float32(biggestContour2)
#     pt4 = np.float32([[0, 0], [widthImg, 0], [0, heightImg],[widthImg, heightImg]])
#     matrix2 = cv2.getPerspectiveTransform(pt3,pt4)
#     imgwrapColored2 = cv2.warpPerspective(img,matrix2,(widthImg,heightImg))
#     # cv2.imshow("Marking Area2",imgwrapColored2)
...
# Apply Threshhold On Box 1
    ...
    # Finding index values Of the marking
    # myindex = []
    # for x in range (0,questions_box1):
    #     arr = myPixelval[x]
    #     # print("arr1", arr)
    #     myIndexval = np.where(arr == np.amax(arr))
    #     # print(myIndexval[0])
    #     myindex.append(myIndexval[0][0])
    # print("Answer Index Of Box 1\n",myindex)
...
#     print("\n")
#     # Apply Threshhold On Box 2
#     imgwrapGray2 = cv2.cvtColor(imgwrapColored2,cv2.COLOR_BGR2GRAY)
#     imgThresh2 = cv2.threshold(imgwrapGray2,170,255,cv2.THRESH_BINARY_INV)[1]

#     # Split boxes
#     boxes = utlis.splitBoxes(imgThresh2)
#     # cv2.imshow("Test Box 1",boxes[0])
...
#     # Getting Non-Zero pixel values of each box
#     myPixelval = np.zeros((questions_box2,Choices))
#     countC = 0
#     countR = 0
...
#     # Finding index values Of the marking
#     myindex2 = []
#     for x in range (0,questions_box2):
#         arr = myPixelval[x]
#         # print("arr2", arr)
#         myIndex2val = np.where(arr == np.amax(arr))
#         # print(myIndex2val[0])
#         myindex2.append(myIndex2val[0][0])
#     print("Answer Index Of Box 2\n",myindex2)
...
#     # Final Marks
#     Correctly_Marked =  (sum(grading) + sum(grading2))
#     print("\nCorrectly Marked: ", Correctly_Marked)

#     # Wrong Answers
#     Wrongly_Marked =  (questions - Correctly_Marked)
#     print("\nWrongly Marked: ",  Wrongly_Marked)

#     # Total Marks
#     marks = Marks_per_question * questions
#     print("\nTotal Marks: ", marks)
...
#     # Percentage
#     score = (Correctly_Marked / questions) * 100 
#     print("\nPercentage: ",score,"%")
...
#     # Displaying Answers For Box 1
#     imgResult1 = imgwrapColored1.copy()
#     imgResult1 = utlis.showAnswers(imgResult1,myindex,grading,ans1,questions_box1,Choices)

#     imgRawDrawing1 = np.zeros_like(imgwrapColored1)
#     imgRawDrawing1 = utlis.showAnswers(imgRawDrawing1,myindex,grading,ans1,questions_box1,Choices)

#     invmatrix1 = cv2.getPerspectiveTransform(pt2,pt1)
#     imgInvwrap1 = cv2.warpPerspective(imgRawDrawing1,invmatrix1,(widthImg,heightImg))

#     # Displaying Answers For Box 2
#     imgResult2 = imgwrapColored2.copy()
#     imgResult2 = utlis.showAnswers(imgResult2,myindex2,grading2,ans2,questions_box2,Choices)

#     imgRawDrawing2 = np.zeros_like(imgwrapColored2)
#     imgRawDrawing2 = utlis.showAnswers(imgRawDrawing2,myindex2,grading2,ans2,questions_box2,Choices)

#     invmatrix2 = cv2.getPerspectiveTransform(pt4,pt3)
#     imgInvwrap2 = cv2.warpPerspective(imgRawDrawing2,invmatrix2,(widthImg,heightImg))
...
#     # cv2.imshow("Marks", imgRawMarks)
#     invMatrixM = cv2.getPerspectiveTransform(ptG2,ptG1)
#     imgInvMarksDisplay = cv2.warpPerspective(imgRawMarks, invMatrixM, (widthImg,heightImg))
#     # cv2.imshow("Obtained Marks",imgInvMarksDisplay)
...
#     # cv2.imshow("Percentage", imgRawPercentage)
#     invMatrixP = cv2.getPerspectiveTransform(ptP2,ptP1)
#     imgInvPercentagDisplay = cv2.warpPerspective(imgRawPercentage, invMatrixP, (widthImg,heightImg))
#     # cv2.imshow("Percentage",imgInvPercentagDisplay)
...
#     # Adding all the images to the Final Image
#     imgFinal = cv2.addWeighted(imgFinal,1,imgInvwrap1,1,0)
#     imgFinal = cv2.addWeighted(imgFinal,1,imgInvwrap2,1,0)
#     imgFinal = cv2.addWeighted(imgFinal,0.6,imgInvMarksDisplay,0.3,0)
#     imgFinal = cv2.addWeighted(imgFinal,1, imgInvGradeDisplay,0.6, 0)
#     imgFinal = cv2.addWeighted(imgFinal,1, imgInvPercentagDisplay,1, 0)

#     # # All the outputs in Single Image
#     # imgFinal = cv2.addWeighted(imgBlank,1,imgInvwrap1,1,0)
#     # imgFinal = cv2.addWeighted(imgFinal,1,imgInvwrap2,1,0)
#     # imgFinal = cv2.addWeighted(imgFinal,0.6,imgInvMarksDisplay,0.3,0)
#     # imgFinal = cv2.addWeighted(imgFinal,1, imgInvGradeDisplay,0.6, 0)
#     # imgFinal = cv2.addWeighted(imgFinal,1, imgInvPercentagDisplay,1, 0)
...
#              [imgwrapColored1,imgwrapColored2, imgResult1,imgResult2, imgThresh1,imgThresh2],
#              [imgRawDrawing1,imgRawDrawing2,imgInvwrap1,imgInvwrap2,imgInvMarksDisplay,imgFinal])
# imageStacked = utlis.stackImages(imageArray,0.3)

# # imageArray1 = ([imgContours,imgBiggestContours])
# # imageStacked1 = utlis.stackImages(imageArray1,0.8)

# # imageArray3 = ([imgResult1,imgResult2])
# # imageStacked3 = utlis.stackImages(imageArray3,0.4)
...
# # cv2.imshow("Biggest Contours", imgBiggestContours)
# # cv2.imshow("Marking Area1",imgwrapColored1)
# # cv2.imshow("Marking Area2",imgwrapColored2)
# # cv2.imshow("Marks Obtained",imgMarksDisplay)
# # cv2.imshow("Percentage Got",imgPercentage)
...
# ####################################################################
# Saving Oputput Images
# cv2.destroyAllWindows()

# Outputs = r"P:\Resume\OMR\Outputs"
# os.chdir(Outputs)
# filename = '1 Final Image.png'
